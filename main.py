...
def create():
    if request.method == "POST":
        ...
        while True:
            ...
            cur = g.db.cursor().execute("SELECT * FROM paste_table where id_sec=?",
                                        (paste_sec,))
...
def show_paste(paste_sec):
    if " " in paste_sec or '"' in paste_sec:
        return "Paste Not Found =("
    cur = g.db.execute('select code, lexer, seen_delete, user_ip from paste_table where id_sec=?',
                       (paste_sec, ))
